...
public class ResultsController {
    ...
    void revealPressed(ActionEvent event) {
        ...
        try{
            ...
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/sys","root","12345");            System.out.println("Connection Established!");
            Statement statement = con.createStatement();
            String query = "select count(*) from votingsystem.vote where party_id = 1";
            ResultSet rs = statement.executeQuery(query);
            rs.next();
            ptiV = rs.getInt(1);
            query = "select count(*) from votingsystem.vote where party_id = 2";
            rs = statement.executeQuery(query);
            rs.next();
            pmlnV = rs.getInt(1);
            query = "select count(*) from votingsystem.vote where party_id = 3";
            rs = statement.executeQuery(query);
            rs.next();
            pppV = rs.getInt(1);
            query = "select count(*) from votingsystem.vote where party_id = 4";
            rs = statement.executeQuery(query);
            rs.next();
            tlpV = rs.getInt(1);
